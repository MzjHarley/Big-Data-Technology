...
集群中的数据节点一般是一个节点运行一个数据节点的进程，负责处理客户端的读写请求，在名称节点的统一调度下进行数据块的创建，删除和重命名等操作。  
每个数据节点实际是保存在本地linux文件系统下的。他们会定期向名称节点发送心跳信息(自己所存储块的列表信息)，报告自己的状态，没有按时发送心跳信息的数据节点会被标记为死机状态，名称不会再
给分配任何I/O请求。  
...
## 体系结构的局限性
+ 命名空间的限制，名称节点保存在内存中，因此命名空间能够容纳对象的个数受到内存空间大小的限制。  
